...
const getTopics = async (req: NextApiRequest, res: NextApiResponse) => {
  ...
  if (!order) {
    try {
      const { rows } = await sql`SELECT id,title,created,modified,url FROM topic order by id asc;`;
    ...
    try {
      const { rows } = await sql`SELECT id,title,created,modified,url FROM topic where id=${temp_order};`;
      return res.status(200).json(rows);
    } catch (err: any) {
      return res.status(500).json({ err });
    }
...
const addTopics = async (req: NextApiRequest, res: NextApiResponse) => {
  try {
    ...
    const result = await sql`INSERT INTO topic (title, detail, created, modified, url) VALUES (${title}, ${detail}, ${created}, ${modified}, ${url});`;
